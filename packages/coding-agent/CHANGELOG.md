...
- Added plan mode: `/plan` (with optional `<description>`) and `alt+o` toggle put the agent into a restricted mode where `write`/`edit` are confined to `~/.pi/draft/<session-id>/` (other paths throw `PlanModeWriteError`), `bash` is disabled, and a new `plan` tool lets the model signal a plan is ready for user confirmation. The active tool set switches to `[read, grep, find, ls, ask, write, edit, plan]` (the `plan` tool is excluded from the default active set and only added on entry). When the model calls `plan({ready: true})`, a confirmation popup shows the draft and offers three choices: execute (exit plan mode, write the final plan to `<cwd>/.pi/<slug>.md`, proceed to implement), refine (stay in plan mode), or new session (fork to a fresh session). State is in-memory only and does not survive `/resume`.
