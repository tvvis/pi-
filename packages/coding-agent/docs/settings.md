...
| `hideStartupExtras` | boolean | `false` | Hide the model scope line, keybinding hints, and onboarding tips at startup. The version line is still shown |
...
`enableInstallTelemetry` controls the anonymous install/update ping to `https://pi.dev/api/report-install`. Use `--offline` or `PI_OFFLINE=1` to disable all startup network operations, including package update checks and install/update telemetry.
