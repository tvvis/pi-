...
- Added per-provider command restrictions to the bash tool. Providers listed in `RESTRICTED_PROVIDERS` (currently `minimax-cn`) are blocked from running `rm` with both recursive and force flags (`-rf`, `-fr`, `-Rf`, `-fR`, `-vrf`, ...). For `git`, the user-approved local writes `git add` and `git commit`, the network subcommand `git fetch`, and the read-only inspection subcommands `branch`, `cat-file`, `describe`, `diff*`, `format-patch`, `grep`, `help`, `log`, `ls-files`, `ls-remote`, `ls-tree`, `reflog`, `remote`, `rev-list`, `rev-parse`, `shortlog`, `show`, `show-branch`, `status`, `tag`, `version`, `whatchanged` are allowed. Mutating flag combinations for `git branch` and `git tag` (`-d`, `-D`, `--delete`, `-m`, `-M`, `--move`, `-c`, `-C`, `--copy`, `-f`, `--force`, `--unset-upstream`) are blocked explicitly so the allowlist does not leak destructive actions through their default read-only form. New `BashToolOptions.getModel?: () => Model` lets the bash tool inspect the active model before each invocation; `AgentSession` wires it to `() => this.model` so model switches apply on the very next call. The matcher splits on shell operators (`;`, `&&`, `||`, `|`, `&`, `$(...)`, backticks) so chained and subshell forms are inspected, but it is intentionally permissive (no shell expansion simulation). New `BashCommandRestrictedError` is thrown with a provider-scoped reason and an explicit "STOP ... wait for the user to intervene" prompt that tells the model to halt the task. `AgentSession` also calls `agent.abort()` and emits a new `command_restricted` session event (`{ command, reason, timestamp }`) on every block, so the in-flight run terminates immediately and UI modes can surface the block to the user without waiting for `turn_end`.

- Added user-authored prompt slot system: a markdown file named `prompts.md`, resolved settings.json-style (`<cwd>/.pi/prompts.md` overrides `~/.pi/agent/prompts.md` at file level), provides optional `## <heading>` sections that override built-in system-prompt sections. Currently two slots are registered (`planMode` and `executePlan`, both case-insensitive heading matches). Bodies may use `${name}` placeholders resolved against per-slot variable maps. Bodies are inserted under the matching `## ` heading on top of a system-managed structural skeleton (tool restrictions, draft root, plan path), so users can customize guidance prose without losing hard guarantees. The slot registry lives in `packages/coding-agent/src/core/prompt-slots.ts`; adding a new slot is a constant-entry plus a `buildSystemPrompt` insertion point.
- Added `AgentSession.setExecutePlan({ planPath, title? })` setter and `executePlan` getter. When set, the system prompt gains an `## Executing Plan` section pointing at the approved plan path; the user's `## Executing Plan` slot body is layered on top. Mutually exclusive with plan mode (UI flow guarantees no overlap).
- The execute-plan context is now persisted to the session file as a new `execute_plan` entry type (written via `SessionManager.appendExecutePlan`, restored as `SessionContext.executePlan` on `/resume`/rebind), so the `## Executing Plan` system-prompt section survives session reload. `AgentSession.setExecutePlan` accepts `{ persist: false }` to restore from the file without appending a duplicate entry. `ExecutePlanEntry` is exported from the session-manager public surface.

- Added `ctrl+q` as a default binding for `app.exit` (exit when editor is empty), alongside `ctrl+d`.
- Added `app.explorer.open` keybinding (default `alt+e`) to open the current project folder in the OS file explorer (Windows `explorer.exe`, WSL converts the path via `wslpath`, macOS `open`, Linux `xdg-open`).
...
- Per-model custom thinking levels: models with `customThinkingLevels` (e.g. DeepSeek V4) now expose their native effort vocabulary directly to the TUI cycle and the provider request, with no pi-level indirection. The TUI shows the configured `label`, the provider sends the configured `value`, and levels not in the model's list are not offered.
- Added `alibaba-token-plan-cn` built-in provider (Alibaba Cloud Model Studio Token Plan, China region). Default model `qwen3.7-max`. Configured via env `ALIBABA_TOKEN_PLAN_API_KEY` or `/login` (auth.json key `alibaba-token-plan-cn`); leave the key blank and pi resolves it from the environment or stored credential at runtime.
- Added `customThinkingLevelsOverrides` setting: a map of model minimatch pattern (same convention as `thinkingLevelMapOverrides`) to a `{label, value}` list that **fully replaces** the model's built-in `customThinkingLevels`. Useful for pointing pi at a proxy that wraps a model with a non-pi effort vocabulary. Resolved via `SettingsManager.getCustomThinkingLevelsOverridesForModel`; applied at model assignment sites in `AgentSession` and in `sdk.ts`'s initial model resolution.
- Added `thinkingLevelMapOverrides` setting: a map of model glob pattern (minimatch, case-insensitive, matched against `provider/modelId` and `modelId`, same convention as `modelAppendSystemPrompts`) to a thinking-level map that is merged into the model's built-in `thinkingLevelMap`. Value shape matches `Model.thinkingLevelMap`: a key set to `null` hides that level from the TUI cycle and the provider request, a string remaps the pi level to a different upstream value, and absent keys fall back to the built-in map. Multiple matching patterns are merged in insertion order (later wins for the same key). Useful for: pointing pi at a proxy that wraps DeepSeek with a different effort vocabulary, hiding `xhigh` from a model where it is unreliable, or forcing a specific upstream value. Resolved via `SettingsManager.getThinkingLevelMapOverridesForModel(provider, modelId)`; applied at model assignment sites in `AgentSession`.
...
- Added ask mode: `--ask` on the CLI, `/ask` in interactive mode, and `alt+a` toggle put the agent into a pure Q&A assistant mode with no file, bash, edit, or write tools available. The system prompt switches to a Q&A identity, active tools are disabled, and any tool call is blocked at runtime. Ask mode is mutually exclusive with plan mode. State is in-memory only and does not survive `/resume`.
- Added plan mode: `/plan` (with optional `<description>`) and `alt+o` toggle put the agent into a restricted mode where `write`/`edit` are confined to `~/.pi/draft/<session-id>/draft.md` (other paths throw `PlanModeWriteError`), `bash` is disabled, and a new `plan` tool lets the model signal a plan is ready for user confirmation. The active tool set switches to `[read, grep, find, ls, ask, write, edit, plan]` (the `plan` tool is excluded from the default active set and only added on entry). When the model calls `plan({ready: true})`, the draft is rendered in the chat for review and a 3-option confirmation popup appears: execute (exit plan mode, write the final plan to `<cwd>/.pi/<slug>.md`, proceed to implement), refine (stay in plan mode), or new session (create a clean new session — no inherited planning conversation — and inject the previous draft's path into the new session's system prompt so the model reads the plan and executes it). State is in-memory only and does not survive `/resume`.
- Added `thinkingLevelsByModel` setting: a per-model thinking level override keyed by `"provider/modelId"`. Takes precedence over `defaultThinkingLevel` when switching to that model. Models without an override fall back to `defaultThinkingLevel` (or the built-in default). When the user changes a model's thinking level via the TUI, the chosen level is written to this map so subsequent switches back to the same model remember it. Switching models no longer carries the current session's level over to the new model.
- Added `ExtensionUIContext.pushChatMarkdown(content, opts)` for appending a markdown message to the chat history. Used by the plan tool to render the plan draft for review, and available to extensions for any purpose.
...
- Added a warning when a response ends truncated (stopReason `length`, i.e. the provider reported `max_tokens` / `incomplete`). Extended thinking can consume the entire output token budget and silently cut the response off with no error; the TUI now shows a warning line and print mode (`pi -p`) prints a warning on stderr, pointing at lowering the thinking level or the `thinkingBudgets` setting. Shared hint text lives in `packages/coding-agent/src/core/hints.ts`.
- Added `--parent-session <path>` and `--parent-relation <fork|plan|subagent>` CLI flags to link a newly created session to a parent session file and record the lineage edge type in its header. Only applies to the new-session path; ignored by `--session`/`--fork`/`--resume`/`--continue`/`--no-session`.
- Added `subagent` to `SessionParentRelation` (alongside `fork` and `plan`). The `/resume` session tree renders subagent child sessions with a `◆` edge tag and gathers all subagent children of a session under a virtual, collapsed-by-default **subagent** group node (display-only, not persisted). Human-created children (fork/plan) are unaffected.
- Added a generic mode to the subagent extension: omit `agent` to run a generic subagent (no named preset; `label` names it in `/resume`). The parent now controls the child's capabilities via new params - `skills` (`--skill`), `noSkills` (`--no-skills`), `systemPrompt` (`--system-prompt`), `appendSystemPrompt` (`--append-system-prompt`), `tools` (`--tools`), and `model` - each settable at the top level and overridable per item in `tasks[]`/`chain[]` (merge priority: item > top-level > agent preset default).
- Added a `type` parameter to the subagent extension: `type: "standard"` (default) runs a generic subagent with no preset, while any other value names a registered agent preset (system prompt + model + tools), equivalent to the existing `agent` parameter. `type` and `agent` are mutually exclusive per scope (top level and each `tasks[]`/`chain[]` item); resolution helpers `presetNameFrom`/`presetDisplayName`/`validateTypeAgentExclusivity` are exported for testing.
- Added `setModel(model)` to `ReplacedSessionContext` (the `ctx` passed to `withSession()` on `newSession()`, `fork()`, and `switchSession()`). `newSession()` re-derives the model from CLI flags + saved default rather than inheriting the current session's model, and the captured `pi`/command `ctx` is stale after replacement, so extensions previously had no way to set the model on the replacement session. `setModel` mirrors plan mode's internal `carryOverModel` flow (capture the current model before the rebind, apply it in `withSession`); returns `false` if no auth is configured for the model.
- The subagent extension now advertises the models configured under `settings.json` `enabledModels` in its tool schema at load time (tool description and the `model` parameter of single/parallel/chain scopes), so the parent model can pass a valid `model` override directly instead of reading settings first.
- Footer now shows the session cache hit rate (`chr X%`) on the right of the pwd line (above the model/stats line), computed as `cacheRead / (cacheRead + input)` from cumulative assistant usage. Only rendered when the session has cache data (`cacheRead` or `cacheWrite` > 0); hidden otherwise, and dropped on very narrow terminals where pwd would not stay meaningful.

### Changed

- Plan mode choices 1 (execute in same session) and 3 (new session) now activate the new `## Executing Plan` system prompt section via `AgentSession.setExecutePlan({ planPath, title? })` instead of injecting a one-line `setSessionNote`. The model sees a structural skeleton with the plan path and any user-authored slot body in every turn of the execution session, instead of a single contextual nudge.
- `AgentSession.setExecutePlan` now persists the execute-plan context to the session file (see the `execute_plan` entry type) so the `## Executing Plan` section survives `/resume` and session rebinds. Previously the context was in-memory only, so a queued execution session (plan-mode option 4) lost its plan context on resume and the model did not know to execute the draft when the user later returned to it.
- `writePlanModeFinal` now returns `{ finalPath, title? }` (was just `string | undefined`) so the caller can pass the plan title through to `setExecutePlan`.
- Plan mode: the confirmation popup no longer embeds the plan draft (was capped at 18 lines and truncated long plans). The draft is now rendered in the chat history before the popup appears, so the user can scroll, copy, and review the full plan in the chat scrollback. The popup is a compact 3-option menu.
- `setExtensionFooter` now manages the bottom panel (where the footer actually lives) instead of the main column, fixing a duplicate-footer rendering bug that appeared whenever an extension swapped or restored the footer (e.g. `/new`, plan mode choice 3, reload).
- Plan tool result is now a single short line (`User chose option N.`) instead of a verbose instruction block. The model already knows what each option means from the system prompt and popup state.
- Sidebar recent file keybindings (`alt+1`–`alt+9`) now open `.html`/`.htm` files in Google Chrome instead of `$VISUAL`/`$EDITOR`; other file types still open in the editor. Platform commands: `chrome` on Windows, `explorer.exe` (with `wslpath`-converted path) in WSL so the Windows file association handles the file (Chrome for Chrome users), `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome` on macOS, `google-chrome` on Linux.
...
- The subagent extension's child processes now persist their own session files (dropped `--no-session`) and link back to the calling session via `--parent-session`/`--parent-relation subagent`, auto-named `subagent:<agent> - <task>`. This makes subagent runs browsable and independently resumable under a virtual "subagent" group in `/resume`. When the parent has no session file (`--no-session`), the child still persists but without parent linkage.
- The subagent extension's `agent` parameter is now optional: named agents are optional presets rather than a required selection, so a parent is no longer forced to pick from the discovered agent directory (previously the only available agent gated every delegation).
- Plan mode system prompt now walks the model through an explicit investigate -> clarify -> draft -> confirm workflow. It must read the relevant code first (not plan from assumptions), then surface and confirm the key open questions / decision points with the user (via `ask` for discrete choices, or by asking in its response) before calling `plan({ready: true})`, so a plan is no longer signaled ready with unresolved assumptions. The `plan` tool's guideline was tightened to match (ready only once the draft is complete AND key questions are confirmed).
- Rewrote the `examples/extensions/handoff.ts` example to deliver the handoff through a forced `write_handoff` tool call (schema-constrained `file`/`prompt`/`summary` fields, with `tool_choice` injected per-provider via `onPayload`) instead of free-text `===PROMPT===`/`===SUMMARY===` markers. The output format is now structurally constrained and no longer depends on the model choosing to emit the markers, which failed on agentic models that stop with `toolUse` intent before producing them. The example also writes the progress doc to `handoff/<name>.md`, links the new session to its parent via `parentSession`, and carries the current model over to the replacement session via `setModel`.

### Fixed

- Cancelling the `ask` dialog (Esc) no longer feeds a "User cancelled the selection" error back to the model as a tool result. The turn now terminates right after the cancel and waits for the user's next input instead of prompting the model to respond to the cancelled question.
...
- Plan mode choice 3 (new session) no longer leaves the new session idle on a stuck "working" spinner. It now automatically starts an execution turn that reads the previous draft and executes it, instead of waiting for user input.
- Plan mode options 3 (new session) and 4 (queue) now carry the planning session's currently selected model into the newly created session instead of re-deriving it from CLI flags / the persisted default. A mid-session `/model` or Ctrl+P switch (notably when pi was started with `--model`) was previously lost on the new session, which fell back to the default model.
